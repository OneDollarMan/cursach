...
async def subscribe(subscription: schemas.SubscriptionRead, user: models.User, s: AsyncSession):
...
async def unsubscribe(subscription: schemas.SubscriptionRead, user: models.User, s: AsyncSession):
    author = await s.get(models.User, subscription.author_id)
    if author is None:
        raise HTTPException(status_code=400, detail="Author doesn't exists")
